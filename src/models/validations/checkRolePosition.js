async function checkRolePosition(interaction,userSelectedObj,commandName){
  ...
  if(userSelectedObj.user.bot){
    await interaction.editReply({content: `I can't ${commandName} a bot!`, ephemeral: true});
    return false;
  }
  
  if(userSelectedId === requestUserId){
    await interaction.editReply({content: `You can't auto ${commandName}!`, ephemeral: true});
    return false;
  }
  ...
  if(targetUserRolePosition >= requestUserRolePosition || userSelectedId === guildOwnerId){
    await interaction.editReply({content: `You can't ${commandName} that user because they have the same/higher role than you or is Server Owner`, ephemeral:true});
    return false;
  }
  
  }
  if(targetUserRolePosition >= botRolePosition || userSelectedId === guildOwnerId){
    await interaction.editReply({content: `I can't ${commandName} that user because they have the same/higher role than me or is Server Owner`, ephemeral:true});
    return false;
  }

...
async function setWarnPunishTypeSettings(
  guild_id,
  punishment_type,
  timeout_duration,
) {
...
async function warnPunishmentCommand(interaction) {
  ...
  const timeout_duration =
    interaction.options.get("timeout_duration")?.value || "0s";

  const nameOfPunishment = punish[punish_type]?.name;

  if (!nameOfPunishment) {
    errorEmbed.setDescription(
      "This punishment type is INVALID! Please select a valid punishment type",
    );
  ...
  try {
    ...
    const success = new EmbedBuilder().setTitle(
      "Warn punishment set successfully!",
    );

    if (punish_type === 3) {
      success.setDescription(
        `You set Punishment type: **${nameOfPunishment}** with Timeout Duration: ${msPretty(
          msDuration,
        )}`,
      );
    } else {
      success.setDescription(
        `You set Punishment type: **${nameOfPunishment}**!`,
      );
    }
    ...
    errorEmbed.setDescription(
      "An error ocurred on executing this command. Please try again later!",
    );
    interaction.editReply({ embeds: [errorEmbed], ephemeral: true });
    pino.error(e);
    return;
